...
void solve() {
    ...
    FOR(i,0,tot){
        if(g[i].sz)mx.insert(*g[i].rbegin());
    }

    while(q--){
        int x,y;
        cin>>x>>y;
        mx.erase(mx.find(*g[b[x]].rbegin()));
        g[b[x]].erase(g[b[x]].find(a[x]));
        if(g[b[x]].sz)mx.insert(*g[b[x]].rbegin());
        b[x]=y;
        if(g[b[x]].sz)mx.erase(mx.find(*g[b[x]].rbegin()));
        g[b[x]].insert(a[x]);
        mx.insert(*g[b[x]].rbegin());
        cout<<*mx.begin()<<endl;
    }

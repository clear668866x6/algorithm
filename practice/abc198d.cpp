...
void solve() {
    ...
    do{
        ...
        FOR(i,0,p.sz-1)s[p[i]-'a']=w[i];

        if(s[a[0]-'a']==0||s[b[0]-'a']==0||s[c[0]-'a']==0)continue;
        int x=0,y=0,z=0;
        for(auto& l:a)x=x*10+(s[l-'a']);
        for(auto& l:b)y=y*10+(s[l-'a']);
        for(auto& l:c)z=z*10+(s[l-'a']);
        if(x+y==z){
            cout<<x<<endl<<y<<endl<<z;
            return;
        }

    }while(next_permutation(w,w+10));

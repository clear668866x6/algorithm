...
namespace Matrix{
    ...
    E operator*(const E& a,const E& b){
        ...
        FOR(i,0,res.n-1){
            FOR(j,0,res.m-1){
                FOR(k,0,a.m-1){
                    res.x[i][j]=(res.x[i][j]+a.x[i][k]*b.x[k][j]%m)%m;
                }
            }
        }
        return res;
...
void solve() {
    ...
    A.x[0][0]=0,A.x[1][0]=a%m,A.x[2][0]=1;
    ...
    FOR(i,1,18){
        ...
        if(mx>a){
            ...
            if(tot){
                ...
                E B;
                B.n=B.m=3;
                B.x[0][0]=mx%m,B.x[0][1]=1,B.x[0][2]=0;
                B.x[1][0]=0,B.x[1][1]=1,B.x[1][2]=b%m;
                B.x[2][0]=0,B.x[2][1]=0,B.x[2][2]=1;
                A=qmi(B,tot)*A;
            }
        }
    }

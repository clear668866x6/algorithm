...
/*
我们发现如果一个数能整除它的所有数位上的数，那么它一定能整除所有这些数位上的数的最小公倍数
（性质1），而1到9的最小公倍数是2520，所有数可能形成的数位上所有数的最小公倍数一定是2520
的因数。（性质2）以它的平方为时间复杂度可以接受。
...
int dfs(int u,int m,int a,bool lim){//m为数位模2520的余数，a为前i位数的lcm
    if(!u)return m%a==0;
    if(!lim&&~f[u][m][w[a]])return f[u][m][w[a]];
    int res=0;
    for(int i=0;i<=(lim?num[u]:9);i++){
        res+=dfs(u-1,(m*10+i)%2520,lcm(a,i),lim&&i==num[u]);
    }
    if(!lim) f[u][m][w[a]]=res;
    return res;
}
...
signed main() {
    ...
    memset(f,-1,sizeof f);
    FOR(i,1,2520){
        if(2520%i==0)w[i]=++cur;//记录2520的因数个数(2520的因数个数只有50个)
    }

...
void solve() {
    ...
    FOR(i,1,n)pre[i]=pre[i-1]^w[i],mp[pre[i]].emplace_back(i);
    ...
    while(m--){
        ...
        auto posx=lower_bound(ALL(mp[pre[y]]),x);
        auto posy=lower_bound(ALL(mp[pre[x-1]]),y);
        if(posy==mp[pre[x-1]].begin()||posx==mp[pre[y]].end()){
            NO;
            continue;
        }
        posy--;//最小的x和最大的y
        if(*posy>*posx&&*posx>=x&&*posy<=y){
            YES;
        }else{
            NO;
        }

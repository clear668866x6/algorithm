...
123. 我们对于一个前缀/后缀整体减一，只会对一个位置的差分值产生影响，加一或减一，那么就利用这个性质一点一点将差分数组变成 0 就可以了。所以以后遇到什么前缀后缀加减操作的情况下，我们就可以考虑差分。
124. 题目如果有存在 $a_i\le n$ ，此时就得想到这个题可能跟下标有关系。
125. 很好的一道图论，类似二分图匹配（以后遇到连边问题，可以考虑把数放在两边考虑）
126. 当看见是图论的问题的时候，如果n比较小的情况下，我们就可以考虑flord。
127. 如果题目说不满足条件的个数，此时我们就得考虑不满足的样子，最好把规模变小。比如是矩阵nxn，那么就变小为2x2类似这样的去分析，这样就会有很独到的分析。
128. 反思：其实已经想到了每层来，也就是按2^k的方式来，已经打算尝试了，但最后没有自己去写，下次得注意一下，有思路了不要看解题
129. abc379d 很好的一道，就是如果我们发现如果于集体修改时间复杂度会很高。因此就得考虑**相对值的方式来做。
130. 求环且包含某个点的最小环，我们可以哟能最短路的思想来解决。但这里会有个问题，求最短路的时候不能重复经过一个点两次，因此我们得让其中一个点设为n+1（也就是不存在的点）。
131. 以后的贡献法的题目如果很复杂，我们不妨列出式子。
132. 这种除2的时间复杂度都是 $log a_i$ 的。
133. 类似abc354c和abc362c，这种贪心题都是我所欠缺的，记住，如果给你两个关系，最好分开来，就是先固定其中的一个顺序，然后去看了另一个顺序，不要试图用什么类似 a>b c>d ac>bd ，这样会很乱的。
134. 算每个连通块的边数可以：
...
```
`t[i]` 表示边数。
135. 并查集的x!=y就是表示不是环的存在。
136. abc387c 数位dp。
137. cf1765N 很好的一道思维。
138. 计算等差数列在区间有多少个数，可以把区间放在全部大于0的部分。要不然会出问题。为什么呢？因为我们算等差数列都是从1开始。
139. gcd是积性函数，所以 gcd(ai*aj,k)/gcd(ai,k)=gcd(aj,k),当i!=j。
140. lxs100b很好的一道不需要枚举i状态的dp（第一次见）
141. 如果有特殊的关系，此时我们就得建立分层图，比如abc325e
142. dp 的题目可以让无效的状态设为-1,dp的值最好和题目要求的东西一致。
...
cf1881d 很好的整体取模的题目
cf1325c 很好的一道构造题，树上mex，给以后的启示：构造题要多画。
cf1671d 很好的一道绝对值和的问题，我们只需要看1和x的插入位置即可，因为：当1和x确定了，其他的数放在相应的位置是无法改变答案的。
1 2 10 6
lxs100b很好的一道不需要枚举i状态的dp（第一次见）

## 值得再做的题
cf1876a 队列
cf1849b 队列，要模一下
abc406c 很好的求波浪型的子串的个数问题
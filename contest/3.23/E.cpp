...
int work(int u,int sum,int mod,int limit){
    if(sum<0)return 0;
    if(!u) return sum==0&&mod==0;
    if(~f[u][sum][mod][limit]) return f[u][sum][mod][limit];
    int res=0;
    for(int i=0;i<=(limit?w[u]:9);i++){
        res+=work(u-1,sum-i,(mod*10+i)%cur,limit&&i==w[u]);
    }
    return f[u][sum][mod][limit]=res;
}
